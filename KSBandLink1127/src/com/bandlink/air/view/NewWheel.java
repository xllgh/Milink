...
public class NewWheel extends View {
	...
	private int mMaxValue = 100, mModType = MOD_TYPE_ONE,
	...
	public NewWheel(Context context, AttributeSet attrs) {
		super(context, attrs);
	...
	private GradientDrawable createBackground() {
		...
		float roundRadius = 2 * mDensity; // 圆角半径
		int strokeColor = getResources().getColor(R.color.blue_ui);// 边框颜色
		// int fillColor = Color.parseColor("#DFDFE0");// 内部填充颜色

		setPadding((int) strokeWidth, (int) strokeWidth, (int) strokeWidth, 0);

		int colors[] = { 0xFFeeeeee, 0xFFFFFFFF, 0xFFeeeeee };// 分别为开始颜色，中间夜色，结束颜色
	...
	public void initViewParam(float defaultValue, int maxValue, int model) {
		defaultValue = Float.valueOf(String.format("%.1f", defaultValue));
		switch (model) {
		case MOD_TYPE_HALF:
			mModType = MOD_TYPE_HALF;
			mLineDivider = ITEM_HALF_DIVIDER;
		...
		case MOD_TYPE_ONE:
			mModType = MOD_TYPE_ONE;
			mLineDivider = ITEM_ONE_DIVIDER;
			mValue = defaultValue*10;
			mMaxValue = maxValue*10;
			break;
		default:
			break;
		}
	...
	private void drawScaleLine(Canvas canvas) {
		...
		linePaint.setColor(getResources().getColor(R.color.blue_ui_a));
			...
			if (xPosition + getPaddingRight() < mWidth) {
				if ((mValue + i) % mModType == 0) {
					canvas.drawLine(xPosition, getPaddingTop(), xPosition,
							mDensity * (int)(ITEM_MAX_HEIGHT*0.6), linePaint);
	...
	private void drawMiddleLine(Canvas canvas) {
		...
		int gap = 12, indexWidth = 4, indexTitleWidth = 24, indexTitleHight = 10, shadow = 3;
	...
	private void notifyValueChange() {
		if (null != mListener) {
			if (mModType == MOD_TYPE_ONE) {
				mListener.onValueChange(Float.valueOf(String.format("%.1f", mValue/10)));
			}
