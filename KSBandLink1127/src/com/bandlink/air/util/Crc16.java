...
public class Crc16 {
	...
	public static int ComputeCrc(int crc, byte[] bytes, int start, int len) {
		for (int i = 0; i < len; i++) {
			crc = (crc >>> 8) ^ table[(crc ^ (0xff & bytes[start + i])) & 0xff];
			crc = crc & 0xffff;
		}
	...
	public static int crc16_compute(int crc, byte[] bytes, int start, int len) {

		for (int i = 0; i < len; i++) {
			crc = (((crc >> 8) & 0xff) | (crc << 8)) & 0xffff;
			crc = (crc ^ (bytes[start + i] & 0xff)) & 0xffff;
