...
/***
 * Air 项目中支持变色的Activity都应继承的Activity基类
 * 
 * @author Kevin
 * 
 */
public abstract class LovefitActivity extends Activity {
	...
	void changeBG(View view) {
		...
		int step = 0;
		try {
			if(steprs!=null && steprs.length>1){
				step = Integer.valueOf(steprs[0].toString());
			}
				...
				va.addUpdateListener(new AnimatorUpdateListener() {
					...
					public void onAnimationUpdate(ValueAnimator animation) {
						...
						LovefitSlidingActivity.changeBarColor(
								LovefitActivity.this, cVal,mainView);
					}
				});
				va.start();
			} catch (Exception e) {
