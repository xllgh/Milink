...
public class SatelliteMenu extends FrameLayout {
	...
	public void onClick() {
		...
		itemClickedListener.mainImgClick();
	}
	...
	public void resetItems(List<SatelliteMenuItem> items) {
		if (menuItems.size() > 0) {
			menuItems.clear();
			this.removeAllViews();
			addItems(items);
	...
	public void resetItemImage(int id, int resId) {

		if (menuItems.size() > 0) {
			try {
				for (SatelliteMenuItem item : menuItems) {
					if (id == item.getId()) {
						item.setImgResourceId(resId);
					}
				}
