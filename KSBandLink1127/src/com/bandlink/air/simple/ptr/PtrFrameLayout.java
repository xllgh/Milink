...
public class PtrFrameLayout extends ViewGroup {
	...
	public PtrFrameLayout(Context context, AttributeSet attrs, int defStyle) {
		...
		if (arr != null) {
			...
			mResistance = arr.getFloat(
					R.styleable.PtrFrameLayout_ptr_resistance, mResistance);
			...
			mRatioOfHeaderHeightToRefresh = arr
					.getFloat(
	...
	private void updatePos() {
		...
		if (mLastPos == POS_START && mCurrentPos != POS_START
				&& mPtrUIHandlerHolder.hasHandler()) {
		...
		if (mStatus == PTR_STATUS_PREPARE) {
			...
			if (mIsUnderTouch && mAutoScrollRefreshTag == 0 && mPullToRefresh
					&& mLastPos < mOffsetToRefresh
					&& mCurrentPos >= mOffsetToRefresh) {
				tryToPerformRefresh();
			}
			// reach header height while auto refresh
			if (mAutoScrollRefreshTag == STATUS_AUTO_SCROLL_LATER
		...
		if (mPtrUIHandlerHolder.hasHandler()) {
			mPtrUIHandlerHolder.onUIPositionChange(this, mIsUnderTouch,
					mStatus, mLastPos, mCurrentPos, oldPercent, currentPercent);
		}
		onPositionChange(mIsUnderTouch, mStatus, mLastPos, mCurrentPos,
				oldPercent, currentPercent);
	}

	protected void onPositionChange(boolean isInTouching, byte status,
			int lastPosition, int currentPosition, float oldPercent,
		...
		if (mStatus == PTR_STATUS_LOADING) {
			...
			if (mKeepHeaderWhenRefresh) {
				...
				if (mCurrentPos > mHeaderHeight && !stayForLoading) {
					mScrollChecker.tryToScrollTo(mHeaderHeight,
							mDurationToClose);
				} else {
					// do nothing
				}
	...
	private void notifyUIRefreshComplete(boolean ignoreHook) {
		/**
		 * after hook operation is done, will call
		 * {@link #notifyUIRefreshComplete} and ignore hook
		 */
		if (mCurrentPos != POS_START && !ignoreHook
	...
	public void autoRefresh(boolean atOnce, int duration) {
		...
		mAutoScrollRefreshTag = atOnce ? STATUS_AUTO_SCROLL_AT_ONCE
				: STATUS_AUTO_SCROLL_LATER;
	...
	private void sendCancelEvent() {
		...
		MotionEvent e = MotionEvent.obtain(
				...
				mDownEvent.getEventTime()
						+ ViewConfiguration.getLongPressTimeout(),
				MotionEvent.ACTION_CANCEL, mDownEvent.getX(),
				mDownEvent.getY(), mDownEvent.getMetaState());
		dispatchTouchEventSupper(e);
	}
