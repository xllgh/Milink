...
public class ReportClient {
	...
	public ReceivedsResult getReceiveds(String msgIds)
			throws APIConnectionException, APIRequestException {
		checkMsgids(msgIds);
		String authCode = ServiceHelper.getAuthorizationBase64(_appKey,
				_masterSecret);

		String url = REPORT_HOST_NAME + REPORT_RECEIVE_PATH + "?msg_ids="
				+ msgIds;
	...
	public MessagesResult getMessages(String msgIds)
		...
		String url = REPORT_HOST_NAME + REPORT_MESSAGE_PATH + "?msg_ids="
				+ msgIds;
		ResponseWrapper response = _httpClient.sendGet(url, null, authCode);
	...
	public UsersResult getUsers(TimeUnit timeUnit, String start, int duration)
		...
		String url = REPORT_HOST_NAME + REPORT_USER_PATH + "?time_unit="
				+ timeUnit.toString() + "&start=" + startEncoded + "&duration="
				+ duration;
		ResponseWrapper response = _httpClient.sendGet(url, null, authCode);
	...
	public static void checkMsgids(String msgIds) {
		...
		if (MSGID_PATTERNS.matcher(msgIds).find()) {
			throw new IllegalArgumentException(
					"msgIds param format is incorrect. "
		...
		try {
			...
			throw new IllegalArgumentException(
					"Every msg_id should be valid Integer number which splits by ','");
		}
	}
