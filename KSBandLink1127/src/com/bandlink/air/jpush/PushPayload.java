 ...
 * 
 * alertAll() is a shortcut for quickly build payload of alert to all platform
 * and all audience; mesageAll() is a shortcut for quickly build payload of
 * message to all platform and all audience.
 * 
 */
public class PushPayload implements PushModel {
	...
	private static final int MAX_GLOBAL_ENTITY_LENGTH = 1200; // Definition
																// acording to
																// JPush Docs
	private static final int MAX_IOS_PAYLOAD_LENGTH = 220; // Definition
															// acording to JPush
															// Docs
	...
	/**
	 * The shortcut of building a simple alert notification object to all
	 * platforms and all audiences
	 */
	public static PushPayload alertAll(String alert) {
		return new Builder().setPlatform(Platform.all())
				.setAudience(Audience.all())
				.setNotification(Notification.alert(alert)).build();
	}

	/**
	 * The shortcut of building a simple message object to all platforms and all
	 * audiences
	 */
	...
	public void resetOptionsApnsProduction(int apnsProduction) {
		if (null == options) {
			options = Options.newBuilder().setApnsProduction(apnsProduction)
					.build();
		} else {
	...
	public boolean isGlobalExceedLength() {
		...
		if (!payload.has(NOTIFICATION)) {
			...
			if (notification.has(AndroidNotification.NOTIFICATION_ANDROID)) {
				JsonObject android = notification
						.getAsJsonObject(AndroidNotification.NOTIFICATION_ANDROID);
	...
	public static class Builder {
		...
		public PushPayload build() {
			Preconditions.checkArgument(
					!(null == audience || null == platform),
					"audience and platform both should be set.");
			Preconditions.checkArgument(
					!(null == notification && null == message),
					"notification or message should be set at least one.");

			// if options is not set, a sendno will be generated for tracing
			// easily
			if (null == options) {
				options = Options.sendno();
			}

			return new PushPayload(platform, audience, notification, message,
					options);
		}
	}
}
