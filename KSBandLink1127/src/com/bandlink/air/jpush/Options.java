...
public class Options implements PushModel {
	...
	private Options(int sendno, long overrideMsgId, long timeToLive,
			int apnsProduction) {
		this.sendno = sendno;
	...
	public static class Builder {
		...
		public Builder setApnsProduction(int apnsProduction) {
			this.apnsProduction = apnsProduction;
			return this;
		}

		public Options build() {
			Preconditions.checkArgument(sendno >= 0,
					"sendno should be greater than 0.");
			Preconditions.checkArgument(overrideMsgId >= 0,
					"override_msg_id should be greater than 0.");
			Preconditions.checkArgument(timeToLive >= 0,
					"time_to_live should be greater than 0.");
			if (sendno <= 0) {
				sendno = ServiceHelper.generateSendno();
			}

			return new Options(sendno, overrideMsgId, timeToLive,
					apnsProduction);
		}
	}
